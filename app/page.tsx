...
export default async function Dashboard(){
 ...
 const authorFilter=activeAuthor?{author:activeAuthor}:{};
 const [week,upcoming]=await Promise.all([
   prisma.content.findMany({where:{scheduledAt:{gte:now,lte:end},...authorFilter},include:{publications:true}}),
   prisma.content.findMany({where:{scheduledAt:{gte:now},...authorFilter},orderBy:{scheduledAt:'asc'},take:6,include:{publications:true}})
 ]);
 const count=(s:ContentStatus)=>week.filter(x=>x.status===s).length;
 const dist=Object.values(Pillar).map(p=>({p,n:week.filter(x=>x.pillar===p).length}));
 const max=Math.max(1,...dist.map(x=>x.n));
 const authors=['LAURO','RENATA','CASAL'] as const;

 return <div className="content"><div className="page-head"><div><div className="eyebrow">Visão executiva · {authorScopeLabel[scope]}</div><h1>O que precisa andar agora.</h1><p>Produção, aprovação e distribuição sem planilha paralela.</p></div><Link className="button" href="/gerador-ideias">Gerador</Link></div>
